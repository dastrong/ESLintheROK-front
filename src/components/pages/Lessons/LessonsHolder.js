...
class LessonsHolder extends PureComponent {
  ...
  componentWillUnmount() {
    clearTimeout(this.loadMsgTimer);
  }

  setData = ({ vocabulary, expressions }) =>
    this.setState({ screen: 3, data: { vocabulary, expressions } });

  getData = (dataSource, settings) => {
    let { baseURL, URLs } = this.state;
    if (dataSource === "books" && URLs.length > 1) {
      URLs = URLs.slice(0, 1);
    }
    if (dataSource === "lessons" && URLs.length > 2) {
      URLs = URLs.slice(0, 2);
    }
    this.setState(
      { loading: true, URLs: [...URLs, settings.url], ...settings },
      () => {
    ...
    this.loadMsgTimer = setTimeout(() => {
      if (!this.state.loading) return;
      this.setState({ message: "Almost up..." });
    }, 3000);
  };
  ...
  changeScreen = ({ screen }) =>
    this.setState({ screen, targetID: null, targetLocation: null });
  ...
  render() {
    ...
    const SecondScreen = (
      <CSSTransition classNames="second-screen" timeout={0}>
    ...
    const ThirdScreen = (
      <CSSTransition classNames="third-screen" timeout={0}>
        <DataHolder
          isAPI={isAPI}
