...
const GridItems = ({ gamesInfo }) =>
  gamesInfo
    ...
    .map(({ router, info }) => {
      const borderTopLeftRadius = info.audio ? "0" : "20px";
      const borderTopRightRadius = info.attachments ? "0" : "20px";
      return (
        <Card key={router.path} style={{ borderTopLeftRadius, borderTopRightRadius }}>
          ...
          <ImgWithPlaceHolder src={info.image} alt={info.title} />
