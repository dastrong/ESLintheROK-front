...
import { handleGameData, handleEvents, handleReset } from "../../helpers/phase1helpers";
import {
  addListeners,
  rmvListeners,
  chooseDataSet,
...
class Stars extends Component {
  ...
  handleClick = e => {
    ...
    e.target.id ? this._handleCardFlip(Number(e.target.id)) : this._handleNextCardUp();
  };
  ...
  handleClasses = i => {
    ...
    return isVocab ? classNames(classes, "vert25") : classNames(classes, "vert33");
  };
  ...
  render() {
    ...
    const cards = gameData.map((card, i) => {
      ...
      const icons = [...Array(Xs[i])].map((x, j) => (
        <Icon name="star" color={starColors[i]} size="small" key={j} />
      ));
      return (
        <Card
          key={i}
