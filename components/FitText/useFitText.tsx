...
// shouldCalculateSolo - true: scale individually; false: everything has the same scale
// delayUpdate - can be used to stop scaling after a certain point, if needed
...
// how many refs need to be created on load
function getNumberOfRefs(gameData: GameDataType) {
  if (typeof gameData === 'object') return gameData.length;
  if (typeof gameData === 'string') return 1;
  return gameData;
}

// handles single and multi worded spans and scales accordingly
function calculateFit(refs: RefsType, shouldCalculateSolo: boolean) {
  ...
  if (multi.length === 0) {
    ...
    return single.forEach(({ ref, pWidth }) =>
...
function getMaxScaleAndWidth(widthsAndScales) {
  return widthsAndScales.reduce((acc, cVal) =>
    acc.scale < cVal.scale ? cVal : acc
  );
}
...
function getRefsAndStuff(refs: RefsType, numLoops: number) {
  ...
  const refsAndStuff = refs.map((ref, i) => {
    const pWidth = pWidths[i];
    const { pHeight, sWidth, sHeight, text } = deRef(ref);
    let width; // to stop typescript errors
    ...
    if (isSingle) {
      ...
      return { ref, pWidth, isSingle, scale, width };
    } else {
      // grab the scale for the min width now to avoid doing it twice later
      const scale = getScale(pWidth, pHeight, minWidth, sHeight);
      const widthsAndScales = [{ width: minWidth, scale }];
      return { ref, pWidth, isSingle, widthsAndScales, width };
    }
  });
...
function getParentWidths(refs: RefsType) {
  // combine all parent widths into one array
  let parentWidths: number[] = refs.map(
    ref => ref.current.parentElement.offsetWidth
  );
